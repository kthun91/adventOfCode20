...
def passwordIsValid(beginIntervall, endIntervall, password, char):
    return checksPasswordForIndex(password, beginIntervall, char) != checksPasswordForIndex(password, endIntervall, char)

#checks, if password has char on certain index
def checksPasswordForIndex(password, index, char):
    return password[int(index)-1] == char
